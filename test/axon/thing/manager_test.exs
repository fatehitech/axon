defmodule Axon.Thing.ManagerTest do
  ...
  test "update_tty_list/2 makes correct tty list when seen nothing yet" do
    ...
    seen_tty = []
    out = update_tty_list(sys_tty, seen_tty)
    assert out == [
      {"/dev/ttyACM0", nil, 0},
      {"/dev/ttyACM1", nil, 0}
    ]
  ...
  test "lost_knowns/2 reveals which known ttys we lost" do
    ...
    out = lost_knowns(prev_tty, new_tty)
    assert out == [
      {"/dev/ttyACM1", "Uno", 2}
    ]
  end
  ...
  test "lost_knowns/2 reveals which known ttys we lost (2)" do
    ...
    out = lost_knowns(prev_ttys, new_ttys)
    assert out == [
  ...
  test "identified/3 updates ttys with the one with given path identified and sends unprobe message" do
    ...
    out = identified(list, "/dev/ttyACM1", "Uno.ino")
    assert out == [
      {"/dev/ttyACM0", nil, 1},
  ...
  test "disconnect/3 removes the element with with the matching tty name and calls the callback with it" do
    ...
    out = disconnect(list, "/dev/ttyACM1", fn(value) ->
      assert value == "world"
    end)
  ...
  test "remove_connected/2" do
    ...
    ttys_out = remove_connected(ttys)
